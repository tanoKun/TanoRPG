...
public class MissionCheck extends Menu {
    ...
    public void onClick(InventoryClickEvent e) {
        ...
        Mission mission = NpcClickListener.meta.get(e.getWhoClicked().getUniqueId());
        if (e.getCurrentItem().getItemMeta().getDisplayName().equals("§a遂行します")){
            new BukkitRunnable() {
                @Override
                public void run() {
                    NpcClickListener.flag.add(e.getWhoClicked().getUniqueId());
                    try {mission.startMission((Player) e.getWhoClicked());} catch (Exception exception) {exception.printStackTrace();}
                    ...
                    NpcClickListener.flag.remove(e.getWhoClicked().getUniqueId());
                    this.cancel();
                }
        ...
        NpcClickListener.meta.remove(e.getWhoClicked().getUniqueId());
        e.getWhoClicked().closeInventory();
    }

    public void onClose(InventoryCloseEvent e) {
        if (NpcClickListener.meta.get(e.getPlayer().getUniqueId()) == null) return;
