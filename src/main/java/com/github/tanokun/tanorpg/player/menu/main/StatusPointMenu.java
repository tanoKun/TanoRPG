...
public class StatusPointMenu implements InventoryProvider {
    ...
    public void init(Player player, InventoryContents contents) {
        ...
        for (StatusType statusType : StatusType.getBasicStatus()) {
            if (statusType.equals(StatusType.HP) || statusType.equals(StatusType.MP)) continue;
            List<String> lore = new ArrayList<>();
            lore.addAll(statusType.getDecArrayList());
            lore.add(" ");
            lore.add("§fクリックで割り振り");
            contents.set(r, c, ClickableItem.of(ItemUtilsKt.createItem(statusType.getMaterial()
